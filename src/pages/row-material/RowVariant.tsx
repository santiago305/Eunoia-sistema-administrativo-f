                ...
                <motion.section initial={shouldReduceMotion ? false : "hidden"} animate={shouldReduceMotion ? false : "show"} variants={fadeUp} className=" bg-white shadow-sm overflow-hidden">
                                ...
                                >
                                    {variants.map((v) => (
                                        <tr key={v.id} className="border-b border-black/5 text-[11px]">
                                            ...
                                            <td className="py-3 px-5 text-black/70">{money(v.price, "PEN")}</td>
                                            <td className="py-3 px-5 text-black/70">{money(v.cost, "PEN")}</td>
                    ...
                    <div className="lg:hidden">
                        <AnimatePresence mode="wait" initial={false}>
                            ...
                            >
                                {variants.map((v) => (
                                    <motion.div key={v.id} variants={shouldReduceMotion ? undefined : item} layout className="rounded-3xl border border-black/10 bg-white p-4 shadow-sm">
                                        <div className="flex items-start justify-between gap-3">
                                            <div className="min-w-0">
                                                ...
                                                <div className="mt-3 flex flex-wrap items-center gap-2">
                                                    <StatusPill active={!!v.isActive} PRIMARY={PRIMARY} />
                                                    <span className="rounded-full bg-black/[0.03] px-2.5 py-1 text-[11px] text-black/70 tabular-nums">Precio: {money(v.price, "PEN")}</span>
                                                    <span className="rounded-full bg-black/[0.03] px-2.5 py-1 text-[11px] text-black/70 tabular-nums">Costo: {money(v.cost, "PEN")}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <IconButton title="Editar" onClick={() => void openEdit(v.id)} PRIMARY={PRIMARY} PRIMARY_HOVER={PRIMARY_HOVER}>
                                ...
                                {!loading && variants.length === 0 && (
                                    <div className="rounded-3xl border border-black/10 bg-white p-4 text-sm text-black/60">No hay variantes con los filtros actuales.</div>
                                )}
                            </motion.div>
                        </AnimatePresence>
                    </div>
                    ...
                    >
                        <div className={"rounded-lg border px-3 py-2 " + (nextActiveState ? "border-teal-200 bg-teal-50" : "border-rose-200 bg-rose-50")}>
                            <p className={"text-sm " + (nextActiveState ? "text-teal-800" : "text-rose-800")}>
                                {nextActiveState ? "Se restaurara la variante nuevamente." : "Se eliminara la variante seleccionada. Esto puede afectar ventas/stock."}
                            </p>
                        </div>
