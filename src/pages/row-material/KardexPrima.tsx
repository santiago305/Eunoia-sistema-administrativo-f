...
export default function KardexPrima() {
  ...
  const loadSearchState = useCallback(async () => {
    try {
      const response = await getInventoryLedgerSearchState({ productType: PRODUCT_TYPE });
  ...
  }, [executedSnapshot, loadSearchState, savingMetric, showFlash]);
  ...
  }, [clearFlash, executedSnapshot, fromDate, loadSearchState, page, showFlash, toDate]);
  ...
  return (
    <PageShell>
      <PageTitle title="Movimientos de materia prima" />

      <div className="grid grid-cols-2 ms:grid-cols-1 gap-3 pt-2 items-center">
        <Headed title="Movimientos de materia prima" subtitle="Auditoría interna del inventario." size="lg" />
      </div>
      ...
      <div className="space-y-3">
        ...
        <DataTable
          ...
          rangeDates={{
            ...
            name: "inventory-ledger-materials-date-range",
