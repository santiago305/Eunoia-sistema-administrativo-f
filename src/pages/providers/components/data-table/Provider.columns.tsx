...
export function getProvidersColumns({
  ...
  onOpenMethods,
  onToggleActive,
}: ProvidersColumnsParams): ColumnDef<Supplier, any>[] {
  const canExpandRows = hasHiddenExpandableFields(
    providerExpandedFields,
    columnVisibility
  );
  ...
  return [
    ...
    columnHelper.display({
      ...
      header: "Proveedor",
      cell: ({ row }) => (
        <div className="text-black/70">{getSupplierDisplayName(row.original)}</div>
      ),
    }),
