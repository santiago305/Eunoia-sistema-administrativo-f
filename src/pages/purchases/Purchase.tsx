...
export default function PurchaseCreateLocal() {
  ...
  const openEquivalenceModal = async (selectedItemId: string) => {
    if (!selectedItemId) return;

    const selectedProduct = products.find((p) => p.itemId === selectedItemId);

    if (!selectedProduct) {
      showFlash(errorResponse("No se encontró el producto seleccionado"));
      return;
    }


    setEquivalenceProductId(selectedItemId);
    setOpenEquivalence(true);
    setPendingItemAfectType(AfectType.TAXED);
    setPendingItemQuantity(1);
    setPendingItemUnitPrice(0);

    await loadEquivalences(selectedItemId);
  };
