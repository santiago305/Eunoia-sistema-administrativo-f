...
import { getProductVariants, listProducts } from "@/services/productService";
...
export default function CatalogProducts() {
  ...
  const [form, setForm] = useState<{ type: ProductType; name: string; description: string; isActive: boolean }>({
  ...
  const queryParams = useMemo(
    ...
    [page, limit, statusFilter, debouncedName, typesFilter]
  );
  ...
  const startCreate = () => {
    setForm({ type:ProductTypes.FINISHED ,name: "", description: "", isActive: true });
    setOpenCreate(true);
  };
  ...
  const startEdit = (productId: string) => {
    ...
    if (!product) return;
    setForm({ type: product.type ,name: product.name, description: product.description ?? "", isActive: product.isActive });
  ...
  const buildCsv = (
    ...
    const header = ["id", "type" , "name", "description", "isActive", "createdAt", "updatedAt"];
  ...
  return (
      <div className="w-full min-h-screen bg-white text-black">
          ...
          <div className="mx-auto w-full max-w-[1500px] 2xl:max-w-[1700px] 3xl:max-w-[1900px] px-4 sm:px-6 lg:px-8 py-6 space-y-6">
              ...
              >
                  <div className="grid grid-cols-1 lg:grid-cols-[minmax(220px,1fr)_180px_280px] gap-3">
                      ...
                      <div className="relative">
                          ...
                          <select
                              ...
                              value={typesFilter}
                              onChange={(event) => {
                                  setTypesFilter(event.target.value as ProductType);
                                  setPage(1);
                              }}
                          >
              ...
              >
                  <div className="flex items-center justify-between px-4 sm:px-5 py-4 border-b border-black/10">
                      <div>
                          ...
                          <p className="text-xs text-black/60">Scroll interno + acciones claras (y sin botones feos gigantes).</p>
                      </div>
                                              ...
                                              <motion.tr key={product.id} variants={shouldReduceMotion ? undefined : item} layout className="border-b border-black/5 hover:bg-black/[0.02]">
                                                  ...
                                                  <td className="py-4 px-5">
                                                      <div className="min-w-0">
                                                          ...
                                                          <p className="text-xs text-black/50 truncate">UUID: {product.id}</p>
                                                      </div>
                                                  </td>

                                                  <td className="py-4 px-5 text-black/70">
                                                      <p className="line-clamp-2 max-w-[680px]">{product.description || "-"}</p>
                                                  </td>
                  ...
                  >
                      <div className="space-y-4">
                          <label className="text-sm">
                              ...
                              >
                                  <option value={ProductTypes.PRIMA}>Materia prima</option>
                                  <option value={ProductTypes.FINISHED}>Producto terminado</option>
                              </select>
                          </label>
                          <label className="text-sm">
                              Nombre
              ...
              <Modal title="Confirmar acción" onClose={() => setDeletingProductId(null)} className="max-w-md">
                  ...
                  >
                      <div className="rounded-2xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-800">
                          <span className="font-semibold">Ojo:</span> estás por cambiar el estado de un producto. Hazlo solo si estás seguro.
                      </div>
