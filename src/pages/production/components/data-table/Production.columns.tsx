...
export function getProductionColumns({
  ...
  return [
    ...
    columnHelper.display({
      ...
      header: "Serie",
      cell: ({ row }) => (
        <div className="text-black/70">{row.original.serie?.code ? 
          `${row.original.serie?.code } - ${row.original.correlative}` : ""}</div>
      ),
    }),
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <div className="text-black/70">{row.original.fromWarehouse?.name ?? "-"}</div>
      ),
    }),
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <div className="text-black/70">{row.original.toWarehouse?.name ?? "-"}</div>
      ),
    }),
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex h-full items-center justify-center">
            {status === "IN_PROGRESS" && (
              <span className="inline-flex rounded-lg bg-slate-50 px-2 py-1 text-[10px] font-medium 
              text-slate-700">
                <TimerToEnd from={nowIso} to={row.original.manufactureDate ?? ""} loadProductionOrders={loadOrders} />
              </span>
            )}
            {status === "PARTIAL" && (
              <span className="flex flex-col items-center rounded-lg bg-slate-50 px-2 py-1 text-[10px] 
