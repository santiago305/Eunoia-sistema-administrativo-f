...
import type { LedgerEntry } from "@/pages/production/types/kardex";
...
export default function KardexProduction() {
  ...
  const [selectedProduct, setSelectedProduct] = useState<PrimaVariant | null>(null);
  ...
  const selectedDocLabel = useMemo(() => {
    ...
    const purchaseSerie = selectedRow.referenceDoc?.purchase?.serie;
    const purchaseCorrelative = selectedRow.referenceDoc?.purchase?.correlative;
    if (purchaseSerie && purchaseCorrelative !== undefined && purchaseCorrelative !== null) {
      return `${purchaseSerie}-${String(purchaseCorrelative).padStart(6, "0")}`;
    }
    const productionDocType = selectedRow.referenceDoc?.production?.docType;
    const productionCorrelative = selectedRow.referenceDoc?.production?.correlative;
    if (productionDocType && productionCorrelative !== undefined && productionCorrelative !== null) {
      return `${productionDocType}-${String(productionCorrelative).padStart(6, "0")}`;
    }
    const docType = selectedRow.document?.docType;
    const correlative = selectedRow.document?.correlative;
    if (docType && correlative !== undefined && correlative !== null) {
      return `${docType}-${String(correlative).padStart(6, "0")}`;
    }
    return selectedRow.docId ?? "-";
  }, [selectedRow]);

  const selectedCounterparty = useMemo(() => {
    if (!selectedRow) return "-";
    const supplier = selectedRow.referenceDoc?.supplier;
    if (supplier) {
      const fullName = [supplier.name, supplier.lastName].filter(Boolean).join(" ").trim();
      return supplier.tradeName ?? fullName ?? supplier.documentNumber ?? "-";
    }
    const user = selectedRow.referenceDoc?.createdBy;
    return user?.name ?? user?.email ?? selectedRow.document?.createdBy ?? "-";
  }, [selectedRow]);

  const selectedProductLabel = useMemo(() => {
    if (!selectedRow) return "-";
    const sku =
      selectedRow.stockItem?.product?.sku ??
      selectedRow.stockItem?.variant?.sku ??
      selectedProduct?.sku ??
      "";
    const name =
      selectedRow.stockItem?.product?.name ??
      selectedProduct?.productName ??
      "";
    if (name) return `${name}${sku ? ` (${sku})` : ""}`;
    return sku || "-";
  }, [selectedRow, selectedProduct]);

  const selectedUnitLabel = useMemo(() => {
    return selectedProduct?.unitName ?? selectedProduct?.unitCode ?? "-";
  }, [selectedProduct]);
            ...
            <FilterableSelect
              ...
              onChange={(value) => {
                ...
                const selected = products.find(
                  (p) => (p.itemId ?? p.id ?? p.primaId ?? "") === value
                );
        ...
        <section className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          <div className="xl:col-span-2 rounded-2xl border border-black/10 bg-white p-5 shadow-sm">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.map((row) => {
                    ...
                    const typeLabel = `${row.document?.docType} - ${row.document?.correlative}`;
                    ...
                    return (
                      <tr
                        key={row.id}
                        className={`border-b border-black/5 cursor-pointer ${selectedRow?.id === row.id ? "bg-black/[0.03]" : ""}`}
                      ...
                      >
                        ...
                        <td className="py-3">
                          <span className={row.direction === "IN" ? "text-emerald-600" : "text-red-600"}>
                            {typeLabel}
                          </span>
                        </td>
                        <td className="py-3 text-right tabular-nums">{entryQty}</td>
                        <td className="py-3 text-right tabular-nums">{exitQty}</td>
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-xs text-black/60">
              <span>Mostrando {startIndex}-{endIndex} de {pagination.total}</span>
          ...
          <div className="space-y-3">
            <div className="rounded-2xl border border-black/10 bg-white p-5 shadow-sm">
              ...
              <div className="mt-2 text-xs space-y-1">
                <p><span className="font-semibold">Documento:</span> {selectedDocLabel}</p>
                <p><span className="font-semibold">Cliente/Proveedor/Usuario:</span> {selectedCounterparty}</p>
                <p><span className="font-semibold">Producto:</span> {selectedProductLabel}</p>
                <p><span className="font-semibold">Unidad base:</span> {selectedUnitLabel}</p>
                <p><span className="font-semibold">Tipo:</span> {selectedRow?.direction ?? "-"}</p>
              </div>
