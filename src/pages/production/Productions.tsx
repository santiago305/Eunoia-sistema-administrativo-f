...
import { Factory, Menu, Plus, Timer, OctagonAlert, FileText, Pencil, Play, Ban, PackageCheck, Filter } from "lucide-react";
...
import { cancelProductionOrder, closeProductionOrder, listProductionOrders, startProductionOrder } from "@/services/productionService";
...
=======
    ...
    const rows = useMemo<ProductionRow[]>(() => {
        return (orders ?? []).map((order) => ({
            id: order.productionId ?? `${order.fromWarehouseId}-${order.toWarehouseId}-${order.createdAt ?? ""}`,
    ...
    const columns = useMemo<DataTableColumn<ProductionRow>[]>(() => {
        return [
            ...
            },
            {
                id: "estado",
                header: "Estado",
                cell: (row) => <span className="inline-flex rounded-lg bg-slate-50 px-2 py-1 text-[10px] font-medium text-slate-700">{row.estado ? (statusLabels[row.estado] ?? "-") : "-"}</span>,
                hideable: false,
                sortable: false,
            },
            {
                        ...
                        <div className="flex h-full items-center justify-center">
                            {order.status === ProductionStatus.IN_PROGRESS && (
                                <span className="inline-flex rounded-lg bg-slate-50 px-2 py-1 text-[10px] font-medium text-slate-700">
                                    <TimerToEnd from={nowIso} to={order.manufactureDate ?? ""} loadProductionOrders={loadOrders} />
                                </span>
                            )}
