...
export type AdviserSearchCapabilities = { canViewOrders: boolean; canViewPerformance: boolean };
...
export function filterAdviserSearchSnapshotByCapabilities(value: Partial<AdviserSearchSnapshot> | null | undefined, capabilities: AdviserSearchCapabilities): AdviserSearchSnapshot {
  const snapshot = sanitizeAdviserSearchSnapshot(value);
  return { ...snapshot, filters: snapshot.filters.filter((rule) => {
    if (rule.field === F.ASSIGNED_ORDERS) return capabilities.canViewOrders || capabilities.canViewPerformance;
    if (rule.field === F.SOLD_TOTAL || rule.field === F.COLLECTED_TOTAL) return capabilities.canViewPerformance;
    return true;
  }) };
}
...
export function buildAdviserSmartSearchFields(catalogs?: AdviserSearchStateResponse["catalogs"] | null, capabilities: AdviserSearchCapabilities = { canViewOrders: true, canViewPerformance: true }): SmartSearchFieldConfig<AdviserSearchField, AdviserSearchOperator>[] { return [
  ...
  ...(capabilities.canViewOrders || capabilities.canViewPerformance ? [{ id: F.ASSIGNED_ORDERS, label: "Pedidos asignados", kind: "number" as const, description: "Compara la cantidad de pedidos.", operators: numberOps }] : []),
  ...(capabilities.canViewPerformance ? [
    { id: F.SOLD_TOTAL, label: "Total vendido", kind: "number" as const, description: "Compara el dinero vendido.", operators: numberOps },
    { id: F.COLLECTED_TOTAL, label: "Total recaudado", kind: "number" as const, description: "Compara el dinero recaudado.", operators: numberOps },
  ] : []),
