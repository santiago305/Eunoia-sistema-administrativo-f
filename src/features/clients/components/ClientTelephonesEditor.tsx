...
export function ClientTelephonesEditor({
  ...
  const submit = () => {
    ...
    setRows((current) => {
      ...
      if (editingId) {
        next = next.map((item) =>
          item.id === editingId ? { ...item, number: trimmed, isActive, isMain } : item,
        );
        ...
        <div className="space-y-1">
          {rows.length ? (
            rows.map((row) => (
              <div
                key={row.id}
                className="flex items-center gap-2 rounded-sm px-2 py-2 hover:bg-slate-50 transition"
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex flex-wrap items-center gap-2 text-[10px]">
                    {row.isMain ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2 py-0.5 text-amber-700 ring-1 ring-inset ring-amber-200 font-medium">
