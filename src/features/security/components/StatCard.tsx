...
export const StatCard = ({ label, value, subtitle, icon: Icon, variant = "default" }: StatCardProps) => (
  <div className={`min-h-[96px] rounded-lg border px-4 py-3 ${variantStyles[variant]}`}>
    <div className="flex items-center justify-between mb-1">
      <span className="text-xs uppercase tracking-wider text-muted-foreground font-medium">{label}</span>
      <Icon className={`h-3.5 w-3.5 ${iconVariantStyles[variant]}`} />
    </div>
    <p className="text-xl font-semibold tracking-tight text-foreground font-mono">{value}</p>
