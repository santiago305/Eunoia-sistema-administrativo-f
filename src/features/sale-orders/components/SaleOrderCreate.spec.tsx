...
vi.mock("@/shared/services/saleOrderService", () => ({
  createSaleOrder: vi.fn(), fetchSaleOrderById: vi.fn(), getSaleOrderPdf: vi.fn(), updateSaleOrder: vi.fn(),
}));
...
vi.mock("@/shared/components/components/FloatingSelect", () => ({
  FloatingSelect: ({ label, onChange }: { label: string; onChange: (value: string) => void }) =>
    label === "Cliente" ? <button onClick={() => onChange("client-1")}>select-client</button> : null,
}));
...
vi.mock("@/features/clients/components/ClientFormModal", () => ({
  ClientFormModal: ({ open, mode, client, onSubmit }: any) => open ? (
    <div data-testid="client-modal">
      <span>{mode}</span><span>{client?.fullName ?? "empty"}</span>
      <button onClick={() => onSubmit({
        type: "NEW", fullName: " Cliente editado ", docType: "DNI", docNumber: " 87654321 ",
        departmentId: "dep", provinceId: "prov", districtId: "dist", address: " Dirección ",
        reference: " Referencia ", isActive: true,
...
describe("SaleOrderCreate client editing", () => {
  beforeEach(() => {
    ...
    getClientByIdMock.mockResolvedValue({ id: "client-1", fullName: "Cliente completo", type: "NEW", docType: "DNI", docNumber: "123", departmentId: "dep", provinceId: "prov", districtId: "dist", isActive: true });
  ...
  it("loads and updates the selected complete client", async () => {
    ...
    await waitFor(() => expect(updateClientMock).toHaveBeenCalledWith("client-1", expect.objectContaining({
