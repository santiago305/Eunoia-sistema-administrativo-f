...
import { BanknoteArrowUp, FileText, Menu, Pencil, Plus, Truck, XCircle } from "lucide-react";
import { DataTable } from "@/shared/components/table/DataTable";
import type { DataTableColumn } from "@/shared/components/table/types";
import { PageShell } from "@/shared/layouts/PageShell";
import { SystemButton } from "@/shared/components/components/SystemButton";
import { SaleOrderModal } from "@/features/sale-orders/components/SaleOrderModal";
import { AgendaStatus, ClientType, DeliveryStatus, DeliveryTypeEnum, type SaleOrder, type SaleOrderSearchSnapshot, type SaleOrderSearchStateResponse } from "@/features/sale-orders/types/saleOrder";
import {
  cancelSaleOrder,
...
export default function SaleOrders() {
  ...
  const handleSaveMetric = useCallback(
    async (name: string) => {
      ...
      try {
        ...
        showFeedback(errorResponse("Error al guardar la métrica."));
        return false;
      } finally {
  ...
  const handleDeleteMetric = useCallback(
    async (metricId: string) => {
      try {
        ...
        showFeedback(errorResponse("Error al eliminar la métrica."));
      }
    },
  ...
  const formatClientLabel = (client: SaleOrder["client"]) => {
    ...
    const deliveryTypeBadge = (type?: SaleOrder["deliveryType"]) => {
      ...
      if (type === DeliveryTypeEnum.ABONADO_ENVIO) return statusBadge("Envío A", "bg-cyan-50 text-cyan-700 ring-cyan-200 rounded-md px-2 py-0.5 text-[9px] font-semibold ring-1 ring-inset");
      return statusBadge("—", "bg-slate-50 text-slate-500 ring-slate-200");
    };
  ...
  return (
    <PageShell>
      ...
      <AlertModal
        ...
        confirmText="Confirmar"
        message="¿Seguro que deseas confirmar la entrega de este pedido?"
